...
setwd("C:/Users/helmi/Documents/study/master/Q1/Mining Software Repositories/assignment/Research/2hot4school")
...
approval <- read.csv("approvals.csv", header = TRUE, sep='|')

# Clean up the approval data
approval$status[approval$status == "MERGED,"] <- "MERGED"

# Get the number of review per core reviewer
coreReviewerList <- approval[approval$r_value < -1 | approval$r_value > 1,]
coreReviewerList <- coreReviewerList[!is.na(coreReviewerList$r_email),]
coreReviewerList <- unique(coreReviewerList$r_email)

coreReviewerMap <- data.frame( "r_email" = character(), "n_review" = integer(), stringsAsFactors=FALSE)
for(i in coreReviewerList) {
  a <- length(which(approval$r_email == i)) 
  newRow <- data.frame(r_email = i , n_review = a)
  coreReviewerMap <- rbind(coreReviewerMap,newRow)
}

# Filter out the 5% of least productive core reviewers
newCoreReviewerMap = coreReviewerMap[coreReviewerMap$n_review > quantile(coreReviewerMap$n_review, 0.05),]
leastCoreReviewer = coreReviewerMap[coreReviewerMap$n_review < quantile(coreReviewerMap$n_review, 0.05),]

# Export to csv
#write.csv(newCoreReviewerMap, file = "newCoreReviewerMap.csv", row.names=FALSE)
#write.csv(leastCoreReviewer, file = "leastCoreReviewer.csv", row.names=FALSE)


# Import data to R
timePositivity <- read.csv("timePositivity.csv", header = TRUE, sep=',')
timePositivity = timePositivity[,c("ID", "BRANCH", "PROJECT", "TIMEINSECOND", "POSITIVITY")]
nrow(timePositivity)

reviewerActivity <- read.csv("reviewerActivity.csv", header = TRUE, sep=',')
nrow(reviewerActivity)

# Merge data
allData = merge(x = reviewerActivity, y = timePositivity, by.x = c("ID", "BRANCH", "PROJECT"), by.y = c("ID", "BRANCH", "PROJECT"), all.y = TRUE)
nrow(allData)
...
# Filter out null positivity
allData = allData[!is.na(allData$POSITIVITY),]
nrow(allData)

# Change positivity value
#allData$POSITIVITY[allData$POSITIVITY == 1]  <- 'r+'
#allData$POSITIVITY[allData$POSITIVITY == -1]  <- 'r-'
...
# Get reviewer activity data, filter null value on R1_ACTIVITY
reviewerActivity = allData[!is.na(allData$R1_ACTIVITY),c("R1_ACTIVITY", "R2_ACTIVITY", "TIMEINSECOND", "POSITIVITY")]

# Generate ordinal data
reviewerActivity$GROUP = as.factor(cut(reviewerActivity$R1_ACTIVITY,quantile(reviewerActivity$R1_ACTIVITY,(0:4)/4), include.lowest=TRUE, labels=LETTERS[1:4]))

# Kruskal-Wallis test
kruskal.test(TIMEINSECOND~GROUP, data=reviewerActivity)

# Mann-Whitney test
pairwise.wilcox.test(reviewerActivity$TIMEINSECOND, reviewerActivity$GROUP, p.adj="bonferroni", exact=F)

# Pairwise comparison
summary(reviewerActivity[which(reviewerActivity$GROUP=='A'),])
summary(reviewerActivity[which(reviewerActivity$GROUP=='B'),])
summary(reviewerActivity[which(reviewerActivity$GROUP=='C'),])
summary(reviewerActivity[which(reviewerActivity$GROUP=='D'),])
